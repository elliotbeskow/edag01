...
struct node_t *initial_node(int m, int n, double **a, double *b, double *c)
{
	...
	for (i = 0; i<n; i++) {
		for (j = 0; j<m; j++)
			memcpy(p->a[j][i], a[j][i], sizeof(double));
		memcpy(p->c[i], c[i], sizeof(double));
	}
	for (i = 0; i<m; i++)
		memcpy(p->b[i], b[i], sizeof(double));
	for (i=0; i < n; i++)
	{
		p->min[i] = -INFINITY; 
		p->max[i] = INFINITY;
...
struct node_t *extend(struct node_t *p, int m, int n, double **a, double *b, double *c, int k, double ak, double bk)
{
	...
	for (i = 0; i<n; i++) {
		memcpy(q->min[i],  p->min[i], sizeof(double));
		memcpy(q->max[i], p->max[i],sizeof(double));
		memcpy(q->c[i], c[i], sizeof(double));
	}
	for (i=0; i<m; i++) {
		for (j=0; j<n; j++)
			memcpy(q->a[i][j], a[i][j], sizeof(double));
		memcpy(q->b[i],  b[i], sizeof(double));
	}
	if (ak > 0) {
...
void bound(struct node_t *p, struct set_t *h, double *zp, double *x, int n)
{
	...
	if (p->z > *zp) {
		*zp = p->z;
		for (i = 0; i < n; i++)
			memcpy(x[i], p->x[i],sizeof(double));
		for (i = 0; i < h->size; i++) {
			if (h->nodes[i]->z < p->z) {
				free_node(h->nodes[i]);
